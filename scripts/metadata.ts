...
import { IArticle, ICategoryItem } from '@/typings'
...
// 获取目标目录下的所有文件
async function getArticles(directory: string): Promise<IArticle[]> {
  try {
    // 获取目录中的所有文件
...
function generateArchiveData(articles: IArticle[]) {
  const categories: ICategoryItem[] = []
  const tags: Record<string, number> = {}
  function record(map: Record<string, number>, key: string) {
    if (!map[key]) map[key] = 0
    map[key]++
  }
  function recordCategories(article: IArticle) {
